...
typedef struct recv_stack
{
    ...
    RECV_BUF* items;             /* stack of stored recv_bufs */
} RSTACK;
 ...
 * @return return the sum of RSTACK size and the size of the data that
 *         items points to.
 */
...
/**
 * @brief initialize the RSTACK member fields.
 * @param RSTACK *p points to the starting addr. of an RSTACK struct
...
int init_shm_stack(RSTACK *p, int stack_size)
{
    ...
    p->size = stack_size;
    p->pos  = -1;
    p->items = (RECV_BUF *) ((char *)p + sizeof(RSTACK));
    return 0;
}

/**
 * @brief create a stack to hold size number of integers and its associated
 *      RSTACK data structure. Put everything in one continous chunk of memory.
 * @param int size maximum number of integers the stack can hold
...
RSTACK *create_stack(int size)
{
    ...
    if ( pstack == NULL ) {
        ...
        char *p = (char *)pstack;
        pstack->items = (RECV_BUF *) ((char *)p + sizeof(RSTACK));
        pstack->size = size;
        pstack->pos  = -1;
    }
 ...
 * @param RSTACK *p the address of the RSTACK data structure
 */
